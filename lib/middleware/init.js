...
var init = function (options, reqProperties, reqPropertiesEssential, router, slave) {
  ...
  var init = async (req, res, next) => { // this function actually enables all the express-like kinkiness ;)
    ...
    const specialMode = req.processingMode || req.app.get('fyrejet mode')

    if (!specialMode && !req.app.specialRoutes[req.path]) { // check that the route is not marked as API-only, with disabled Express req & res additions.
      return expressServe()
    }

    if (specialMode === 'properties as functions' || req.app.propsAsFnsRoutes[req.path]) {
    ...
    req.processingMode = specialMode
