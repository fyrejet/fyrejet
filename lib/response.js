...
if (process.env.UWS_SERVER || process.env.UWS_SERVER_ENABLED_FOR_TEST) {
  ...
  proto = uwsCompat.uwsHttpResponse.prototype
}
...
function resSend (res, body) { // split to avoid deprecation checks when returning from resJson or res.json
  const req = res.req
  ...
  if (body !== undefined && !req.rData_internal.noEtag && !res.getHeader('ETag')) {
    let etag
    // determine if ETag should be generated
    const etagFn = app.__settings.get('etag fn')

    // populate ETag
    if (etagFn && (etag = etagFn(body))) { // unlike express, we don't need to check if there is len, because we know it for certain, since we placed this if block in a different place
  ...
  if ((fresh && (res.statusCode = 304)) || res.statusCode === 204 || res.statusCode === 304) {
  ...
  res.end(req.rData_internal.method === 'HEAD' ? null : body)
  return res
}
...
function resJson (res, val) { // this is split, so we can avoid checks for deprecated functionality
  // settings
  const app = res.app
