...
res.send = function send (body) {
  ...
  if (typeof chunk === 'number' || arguments.length === 2) {
    ...
    if (arguments.length === 1) {
      ...
      if (!this.getHeader(CONTENT_TYPE_HEADER)) {
        ...
        this.setHeader(CONTENT_TYPE_HEADER, TYPE_PLAIN)
      }
  ...
  switch (typeof chunk) {
    ...
    case 'object':
      ...
      return resJson(this, chunk) // no need to go through checks for deprecated functionality
  }
  return resSend(this,chunk)
}

function resSend (res, chunk) { // split to avoid deprecation checks when returning from resJson or res.json
  ...
  if (typeof chunk === 'string' && !req.rData_internal.encodingSet) {
  ...
  if (chunk !== undefined && !req.rData_internal.noEtag && !res.getHeader('ETag')) {
    var etag
