...
module.exports = class Trouter {
  ...
  use (route, ...fns) {
    ...
    this.routes.push({ keys, pattern, path: route, method: '', handlers, handlersArgsNum, starCatchAll, middleware: true, init, api, propsAsFns, noEtag, guid: uuidv4() })
    return this
  }
  ...
  add (method, route, ...fns) {
    ...
    const handlers = [].concat.apply([], fns)
    this.routes.push({ keys, pattern, path: route, method, handlers, handlersArgsNum, starCatchAll, api, propsAsFns, noEtag, guid: uuidv4() })
    return this
  }
  ...
  find (method, url, req, res) {
    ...
    let handlers = []; let handlersArgsNum = []; const routes = []; let specialMode = false; let noEtag = false
    for (; i < arr.length; i++) {
      tmp = arr[i]
      if (tmp.method.length === 0 || tmp.method === method || (isHEAD && tmp.method === 'GET')) {
        ...
        if (test !== null) {
          ...
          if (!specialMode) {
            if (tmp.api) specialMode = 'api'
            else if (tmp.propsAsFns) specialMode = 'properties as functions'
          }
          if (!noEtag) {
