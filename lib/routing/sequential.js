...
module.exports = (config = {}) => {
  ...
  router.lookup = (req, res, step) => {
    ...
    if (!step) {
      ...
      let split = req.rData_internal.url.split('?')
      req.path = split[0]
    ...
    const urlBackup = req.rData_internal.url
    req.rData_internal.url = req.rData_internal.url.replace(req.rData_internal.lastPattern, '').trim() || urlBackup

    let split = req.rData_internal.url.split('?', 1) // this is NOT repetitive code as req.rData_internal.url value is now different here
    req.path = split[0] 
    ...
    function carryOn() {
      const reqCacheKey = req.rData_internal.method + req.path
      let match = cache.get(reqCacheKey)
      if (!match) {
        match = router.find(req.method, req.path, req, res)
        match.urlProperVerified = fastDecode(req.path) ? true : false
      ...
      const middlewares = [...match.handlers, ...fnArr]
