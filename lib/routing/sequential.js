...
module.exports = (config = {}) => {
  ...
  router.lookup = (req, res, step) => {
    ...
    if (!step) {
      ...
      const split = req.rData_internal.url.split('?')
      req.path = split[0]
    ...
    const split = req.rData_internal.url.split('?', 1) // this is NOT repetitive code as req.rData_internal.url value is now different here
    req.path = split[0]
