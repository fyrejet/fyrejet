...
module.exports = (config = {}) => {
  ...
  router.lookup = (req, res, step) => {
    ...
    function init () {
      if (!req.rData_internal) {
        ...
        try {
          ...
          req.rData_internal.urlProperVerified = true
        } catch (e) {
          req.rData_internal.urlProperVerified = false
        }
    ...
    function routeFurther (req, res, step, match) {
      ...
      if (justInitialized) return next.initMiddleware(middlewares, middlewaresArgsNum, req, res, 0, 0, routers, config.defaultRoute, config.errorHandler, null)
      return next.next(middlewares, middlewaresArgsNum, req, res, 0, 0, routers, config.defaultRoute, config.errorHandler, null)
    }
  }
