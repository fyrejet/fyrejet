...
module.exports = class Trouter {
  ...
  use (route, ...fns) {
    ...
    this.routes.push({ keys, pattern, path: route, method: '', handlers, handlersArgsNum, starCatchAll, middleware: true, init })
    return this
  }
  ...
  add (method, route, ...fns) {
    ...
    const handlers = [].concat.apply([], fns)
    this.routes.push({ keys, pattern, path: route, method, handlers, handlersArgsNum, starCatchAll })
    return this
  }
  ...
  find (method, url, req, res) {
    ...
    let handlers = []; let handlersArgsNum = []; const routes = []
    for (; i < arr.length; i++) {
      tmp = arr[i]
    ...
    const z = { handlers, handlersArgsNum, url, sRoutes: routes }
    return z
  }
}
