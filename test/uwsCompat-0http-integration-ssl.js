...
if (process.env.UWS_SERVER_ENABLED_FOR_TEST) {
  describe("uwsCompat - 0http Web Framework - Integration with SSL", () => {
    const baseUrl = "https://localhost:" + 9999;

    const { router, server } = require("0http")({
      server: require("../lib/uwsCompat").uwsCompat({
        cert_file_name: path.join(__dirname, "./tmp/test.crt"),
        key_file_name: path.join(__dirname, "./tmp/test.key"),
        passphrase: "test",
      }),
    });
      ...
      await request(baseUrl)
        ...
        .then((response) => {
          expect(response.text).to.equal(
            JSON.stringify({ name: "0http", dep: ["a", "b"] })
          );
        });
    });
    ...
    it("should retrieve file using pipe", async () => {
      await request(baseUrl)
        ...
        .then((response) => {
          ...
          expect(
            response.text.indexOf("should retrieve file using pipe") > 0
          ).to.equal(true);
        });
    });
