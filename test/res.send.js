...
describe('res', function () {
  ...
  describe('.send(number, string, object)', function () {
    it('should send number as json', function (done) {
      var app = express()

      app.use(function (req, res) {
        res.status(200).send(0.123, 'foo', {foo: "bar"})
      })

      request(app)
        .get('/')
        .expect('Content-Type', 'application/json; charset=utf-8')
        .expect(200, '[0.123,"foo",{"foo":"bar"}]', done)
    })
  })
    ...
    describe('when enabled', function () {
      ...
      methods.forEach(function (method) {
        ...
        it('should send ETag in response to ' + method.toUpperCase() + ' request', function (done) {
          ...
          if (method === 'all') method = 'get' // slight modification, because there is no such thing as 'ALL' HTTP method in real life ;)
          request(app)
            [method]('/')
            .expect('ETag', 'W/"c-IgR/L5SF7CJQff4wxKGF/vfPuZ0"')
            .expect(200, done)
