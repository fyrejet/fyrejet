...
import { FyrejetRequest, FyrejetResponse } from "./types"
import proto from './application'
import {EventEmitter} from 'events'
import {serveStatic}  from "./additions/static"
...
const res = require('./response').res
const bodyParser = require('body-parser')

const requestRouter = require('./routing/request-router')
...
const defaultErrorHandler = (err: unknown, req: FyrejetRequest, res: FyrejetResponse) : void => {
...
function createApplication (options = {}) {
  ...
  if (options.serverType === 'uWebSocket' || options.serverType === 'uWebSockets' || process.env.UWS_SERVER_ENABLED_FOR_TEST === 'TRUE' || process.env.UWS_SERVER === 'TRUE') { // legacy or tests
    ...
    options.server = require('./uwsCompat').uwsCompat(options)
...
exports.uwsCompat = require('./uwsCompat').uwsCompat
